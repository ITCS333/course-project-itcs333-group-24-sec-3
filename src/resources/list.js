...
const updateCountBadge = (visibleCount, totalCount = resources.length) => {
  ...
  if (visibleCount === totalCount) {
    resourceCountBadge.textContent = `${visibleCount} resource${visibleCount === 1 ? '' : 's'}`;
    return;
  }
...
const createResourceArticle = (resource) => {
    ...
    <div class="card h-100 shadow-sm border-0">
      <div class="card-body d-flex flex-column">
        <div class="d-flex justify-content-between gap-2">
          ...
          <span class="badge text-bg-light align-self-start">${new Date(resource.created_at).toLocaleDateString()}</span>
        </div>
        <p class="text-muted flex-grow-1">
          ${escapeHtml(truncate(resource.description || 'No description provided.'))}
        </p>
        <div class="mt-3 d-flex flex-wrap gap-2">
          <a
            href="${safeLink ?? '#'}"
            class="btn btn-outline-secondary ${safeLink ? '' : 'disabled'}"
            ${safeLink ? 'target="_blank" rel="noopener"' : 'aria-disabled="true"'}
          >
            Open Resource
          </a>
          <a
...
const loadResources = async () => {
  ...
  try {
    ...
    if (!resources.length) {
      showFeedback('No resources have been shared yet. Check back soon!', 'info');
    }
    ...
    showFeedback('Sorry, we could not load the resources right now. Please try again later.', 'danger');
    listSection.innerHTML = `
      <div class="col-12">
